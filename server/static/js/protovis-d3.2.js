...
pv.Force.spring = function(k) {
  ...
  force.links = function(x) {
    if (arguments.length) {
      links = x;
      kl = x.map(function(l) {
			  var constant = 1 / Math.sqrt(Math.max(
												l.sourceNode.linkDegree,
												l.targetNode.linkDegree));
			 
			  if (l.sourceNode.group == l.targetNode.group ||
				  l.sourceNode.group == 'COMMON' ||
				  l.targetNode.group == 'COMMON') {
				  return constant;
			  }
			  else
				  return 0.0;//constant * constant;
        });
      return force;
    }
    return links;
